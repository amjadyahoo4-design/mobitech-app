...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Tajawal', sans-serif; }
        ...
        .toast-notify { position: fixed; bottom: 30px; left: 50%; transform: translateX(-50%) scale(0.9); background: #1e293b; backdrop-filter: blur(12px); padding: 12px 24px; border-radius: 50px; display: flex; align-items: center; gap: 12px; z-index: 2000; border-right: 4px solid var(--accent-color); opacity: 0; transition: 0.2s; pointer-events: none; font-weight: 500; box-shadow: 0 8px 20px rgba(0,0,0,0.4); }
...
<body>
    ...
    <section class="hero">
        ...
        <div class="social-buttons">
            ...
            <a href="https://t.me/+96181157961" class="btn btn-telegram" target="_blank">
    ...
    <script>
        ...
        async function fetchProductsFromServer() {
            try {
                ...
                showToast("حدث خطأ في تحميل البيانات من السيرفر", true);
            }
        }
        ...
        function showAllStore() {
            currentFilter = 'الكل'; searchQuery = '';
        ...
        function refreshStoreUI() {
            ...
            approvedItems.forEach(item => {
                if (currentFilter !== 'الكل' && item.category !== currentFilter) return;
                if (searchQuery.trim() !== '' && !item.title.includes(searchQuery) && !item.desc.includes(searchQuery)) return;
...
// مسارات الباكيند (API Routes) لمعالجة وحفظ البيانات وثبات المنتجات حياً لكل المستخدمين
